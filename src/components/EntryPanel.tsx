...
export function EntryPanel({ locale, showId }: Props) {
  ...
  return (
    <div className="surface-card ring-brand space-y-4 rounded-[2rem] p-5">
      <form className="flex flex-col gap-3 md:flex-row" onSubmit={submitNumber}>
        <input
          className="min-w-0 flex-1 rounded-full border border-slate-200 bg-white px-5 py-3 text-base text-slate-900 outline-none ring-0 placeholder:text-slate-400 focus:border-[color:var(--brand-accent)] dark:border-white/12 dark:bg-slate-900/85 dark:text-white dark:placeholder:text-slate-500"
          inputMode="numeric"
